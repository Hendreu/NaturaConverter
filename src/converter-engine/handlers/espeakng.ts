...
import type { SimpleTTS } from "./espeakng.js/js/espeakng-simple.js";
...
export class espeakngHandler implements FormatHandler {
  ...
  #SimpleTTSClass: typeof SimpleTTS | undefined = undefined;
  ...
  async getTTS(): Promise<SimpleTTS> {
    if(this.#tts == undefined) {
      const { SimpleTTS } = await import("./espeakng.js/js/espeakng-simple.js");
      this.#SimpleTTSClass = SimpleTTS;
    ...
    return Promise.all(inputFiles.map(async(file) => {
      const audio = await new Promise<AudioBuffer>(resolve => {
        tts.speak(new TextDecoder().decode(file.bytes), (audio: Float32Array, sampleRate: number) => {
          resolve(this.#SimpleTTSClass!.createAudioBuffer(audio, tts.sampleRate) as AudioBuffer);
        })
      });
